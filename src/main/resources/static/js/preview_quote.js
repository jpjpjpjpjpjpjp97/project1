...
preview_modal.addEventListener('click', (event) => {
    ...
    quote_total_price.innerText = coverages.reduce((totalValue, coverage) => car_value*coverage.minimum_percentage/100 > coverage.price ? totalValue+parseInt(car_value*coverage.minimum_percentage/100) : totalValue+parseInt(coverage.price), 0);
});
